...
SELECT TRIM(LEADING '.' FROM TRIM(a.name)) AS value, COUNT(*)::BIGINT AS record_count
FROM nsr n, unnest(string_to_array(n.authors, ';')) AS a(name)
WHERE TRIM(LEADING '.' FROM TRIM(a.name)) <> ''
GROUP BY TRIM(LEADING '.' FROM TRIM(a.name))
ORDER BY TRIM(LEADING '.' FROM TRIM(a.name));
