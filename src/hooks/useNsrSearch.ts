...
async function textSearch(query: string): Promise<NsrRecord[]> {
  ...
  const { data, error } = await supabase
    ...
    .or(`key_number.ilike.${pattern},title.ilike.${pattern},authors.ilike.${pattern},reference.ilike.${pattern},keywords.ilike.${pattern},nuclides::text.ilike.${pattern},reactions::text.ilike.${pattern}`)
